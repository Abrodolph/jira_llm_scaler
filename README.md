...
This script will connect to the Apache Jira API and start scraping all issues defined in `config.py`.

```bash
python scraper.py
```

_(Note: This will take a very long time, as it's scraping thousands of issues politely.)_
...
This will read `config.RAW_DATA_FILE` and create `config.LLM_DATA_FILE`.
...
│
├── config.py          # Central config for projects, filepaths, and API settings
...
└── sample_llm.jsonl   # An "AFTER" sample of the final, clean LLM data
```

_(Runtime files like `scraper.log`, `transform.log`, and `checkpoint.json` will be created when the scripts are run.)_
...
```bash
{"key": "SPARK-54134", "fields": {"summary": "Optimize Arrow memory usage", "description": "We have encountered OOM...", "comment": {"comments": [], ...}}}
{"key": "KAFKA-19856", "fields": {"summary": "Stabilize ClientUtilsTest...", "description": "We receive a bunch of mails...", "comment": {"comments": [{"body": "[~mimaison] it seems the CI is...", ...}]}}}
{"key": "HADOOP-19736", "fields": {"summary": "ABFS: Support for new auth type...", "description": "Adding support for new authentication...", "comment": {"comments": [{"body": "manika137 opened a new pull request...", ...}]}}}
```
